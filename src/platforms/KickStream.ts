...
export interface KickConfig extends StreamConfig {
  streamKey: string;
  rtmpUrl?: string;
}

export class KickStream extends BaseStream {
  ...
  constructor(config: KickConfig) {
    ...
    this.rtmpUrl = config.rtmpUrl || "rtmp://ingest.kick.com/live";

    if (!this.streamKey) {
      throw new Error("Kick stream key is required");
    }
  }
